...
app.put('/competicoes/:id/jogadores', async (req, res) => {
  const competicao_id = Number(req.params.id);
  const { jogador_ids } = req.body as { jogador_ids: number[] };
  ...
  try {
    const partida = await prisma.partida.create({
      data: {
        ...
        categoria_id: Number(categoria_id),
        competicao_id: competicao_id ? Number(competicao_id) : null,
...
app.get('/partidas', async (req, res) => {
  try {
    ...
    if (req.query.competicao_id) where.competicao_id = Number(req.query.competicao_id);
