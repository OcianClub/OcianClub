...
import { View, Text, TouchableOpacity, ScrollView, Modal, ActivityIndicator, Alert, Image, FlatList } from 'react-native';
...
import { atualizarStatusPartida, atualizarPlacarPartida, criarEvento } from '@/src/services/api';
...
function LogoTime({ uri, size = 56 }: { uri: string | null; size?: number }) {
  ...
  return <View style={{ width: size, height: size, borderRadius: 10, backgroundColor: '#1e1e1e', alignItems: 'center', justifyContent: 'center' }}><MaterialCommunityIcons name="shield-outline" size={size * 0.5} color="#333" /></View>;
}
...
export default function DetalhesPartida({ partida: partidaInicial, isAdmin, onBack }: Props) {
  ...
  const iniciarPartida = async () => {
    ...
    if (qtdeTitulares !== 5) {
      Alert.alert('Atenção', `Para iniciar a partida, é obrigatório definir exatamente 5 titulares. Atualmente há ${qtdeTitulares}. Clique em ALTERAR na seção Escalação.`);
      return;
    }
    try {
    ...
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        ...
        {isAdmin && partida.status === 'FINALIZADA' && (
          <TouchableOpacity style={[styles.headerEditBtn, modoEdicao && { backgroundColor: colors.primary, borderColor: colors.primary }]} onPress={() => setModoEdicao(!modoEdicao)}>
            <Text style={[styles.headerEditBtnText, modoEdicao && { color: '#fff' }]}>{modoEdicao ? 'CONCLUIR' : 'EDITAR DADOS'}</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoBar}>
          <View style={styles.infoBarItem}><MaterialCommunityIcons name="map-marker-outline" size={16} color={colors.text_secondary} /><Text style={styles.infoBarLabel}>Local</Text><Text style={styles.infoBarValue} numberOfLines={1}>{partida.local ?? 'Não definido'}</Text></View>
          <View style={styles.infoBarDivider} />
          <View style={styles.infoBarItem}><MaterialCommunityIcons name="clock-outline" size={16} color={colors.text_secondary} /><Text style={styles.infoBarLabel}>Horário</Text><Text style={styles.infoBarValue}>{partida.horario ?? '--:--'}</Text></View>
          <View style={styles.infoBarDivider} />
        ...
        {isInterativo && (
          <>
            <View style={styles.section}>
              <View style={styles.sectionHeader}><View style={styles.sectionTitleRow}><View style={styles.sectionBar} /><Text style={styles.sectionTitle}>PERÍODO (TEMPO)</Text></View></View>
            ...
            <View style={styles.section}>
              <View style={styles.sectionHeader}><View style={styles.sectionTitleRow}><View style={styles.sectionBar} /><Text style={styles.sectionTitle}>REGISTRAR EVENTO NO SCOUT</Text></View></View>
              <View style={styles.eventsGrid}>
                <TouchableOpacity style={styles.golCard} activeOpacity={0.8} onPress={() => setModalEvento({ tipo: 'GOL', label: 'Gol do Jogador' })}>
                  <View style={styles.golLeft}><View style={styles.golIconWrap}><MaterialCommunityIcons name="soccer" size={22} color={colors.primary} /></View><View><Text style={styles.golLabel}>Gol</Text><Text style={styles.golSub}>Selecionar jogador →</Text></View></View><MaterialCommunityIcons name="chevron-right" size={22} color={colors.primary} />
                </TouchableOpacity>
                <View style={styles.eventsRow}>{EVENTO_CARDS.slice(0, 2).map(card => (<TouchableOpacity key={card.tipo} style={styles.eventCard} activeOpacity={0.8} onPress={() => setModalEvento({ tipo: card.tipo, label: card.label })}><View style={styles.eventIconWrap}>{card.icon}</View><Text style={styles.eventLabel}>{card.label}</Text></TouchableOpacity>))}</View>
                <View style={styles.eventsRow}>{EVENTO_CARDS.slice(2, 4).map(card => (<TouchableOpacity key={card.tipo} style={styles.eventCard} activeOpacity={0.8} onPress={() => setModalEvento({ tipo: card.tipo, label: card.label })}><View style={styles.eventIconWrap}>{card.icon}</View><Text style={styles.eventLabel}>{card.label}</Text></TouchableOpacity>))}</View>
                <TouchableOpacity style={styles.eventCardCenter} activeOpacity={0.8} onPress={() => setModalEvento({ tipo: EVENTO_CARDS[4].tipo, label: EVENTO_CARDS[4].label })}><View style={styles.eventIconWrap}>{EVENTO_CARDS[4].icon}</View><Text style={styles.eventLabel}>{EVENTO_CARDS[4].label}</Text></TouchableOpacity>
              </View>
            </View>
          </>
        )}
          ...
          <View style={[styles.modalSheet, { maxHeight: '85%' }]}>
            ...
            {!jogadorEvento ? (
              <>
                ...
                <Text style={{ fontFamily: 'Creato-Regular', color: colors.text_secondary, fontSize: 13, marginBottom: 16 }}>Quem fez?</Text>
                {escalacao.length === 0 ? (
                  <View style={{ alignItems: 'center', paddingVertical: 24, gap: 8 }}><MaterialCommunityIcons name="account-off-outline" size={40} color="#333" /><Text style={{ fontFamily: 'Creato-Regular', color: '#555', fontSize: 13, textAlign: 'center' }}>Nenhum jogador cadastrado na súmula.{'\n'}Defina a escalação primeiro.</Text></View>
                ) : (
                  <FlatList
                    data={escalacao}
                    ...
                    renderItem={({ item }) => (
                      <TouchableOpacity style={[styles.jogadorSelectRow, { marginBottom: 8 }]} onPress={() => setJogadorEvento(item)}>
                        ...
                        <View style={{ flex: 1 }}><Text style={styles.jogadorSelectNome}>{item.jogador.nome}</Text><Text style={styles.jogadorSelectPos}>{item.jogador.posicao} {item.titular ? '' : ' (Banco)'}</Text></View>
                        <MaterialCommunityIcons name="chevron-right" size={20} color={colors.text_secondary} />
                      </TouchableOpacity>
                    )}
                  />
                )}
                <TouchableOpacity style={{ marginTop: 12, alignItems: 'center', paddingVertical: 12 }} onPress={() => setModalEvento(null)}><Text style={{ fontFamily: 'Creato-Bold', color: colors.text_secondary, fontSize: 13 }}>CANCELAR</Text></TouchableOpacity>
              </>
            ) : (
              <>
      ...
      <Modal visible={modalFinalizar} transparent animationType="fade" onRequestClose={() => setModalFinalizar(false)}>
        <View style={styles.confirmOverlay}>
          <View style={styles.confirmBox}>
            ...
            <Text style={styles.confirmDesc}>Os dados serão salvos e o Scout de IA será atualizado com os eventos desta partida. Você poderá editar os dados depois se necessário.</Text>
